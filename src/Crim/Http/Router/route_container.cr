...
module Crim::Http::Router
  ...
  class RouteContainer
    ...
    def any(
      ...
      if !all_methods
        add(route, action, "get", regex, default)
        add(route, action, "post", regex, default)
      else
        METHODS.each do |m|
          add(route, action, m.downcase.strip, regex, default)
        end
      end
    end
    ...
    {% for method in METHODS %}
      ...
      def {{method.id.downcase.strip}}(
        ...
        add(route, action, method, regex, default)
      end
    {% end %}
